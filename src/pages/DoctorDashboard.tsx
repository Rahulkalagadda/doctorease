...
import {
  ...
  Upload, Settings, Bell, Search, LogOut, DollarSign, Users, Award, Clock, ArrowUpRight, PlayCircle
} from "lucide-react";
...
export default function DoctorDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "patients" | "appointments" | "earnings" | "settings">("dashboard");
  ...
  const [appointments, setAppointments] = useState([
    { id: "1", patient: "Jane Doe", age: "28 yrs", time: "03:40 PM", status: "Active", reason: "Fever & severe headache", roomId: "doctor_45_patient_88_2026" },
    { id: "2", patient: "Rahul Gupta", age: "42 yrs", time: "04:30 PM", status: "Scheduled", reason: "Follow-up on ECG report", roomId: "doctor_45_patient_12_2026" },
    { id: "3", patient: "Priya Sen", age: "31 yrs", time: "05:15 PM", status: "Scheduled", reason: "Fatigue & thyroid review", roomId: "doctor_45_patient_19_2026" }
  ]);

  // Patients State
  const [patients, setPatients] = useState([
    { id: "P-88", name: "Jane Doe", sex: "Female", age: "28", email: "jane@example.com", phone: "+91 98765 43210", lastVisit: "Today" },
    { id: "P-12", name: "Rahul Gupta", sex: "Male", age: "42", email: "rahul@gupta.com", phone: "+91 99887 76655", lastVisit: "May 10, 2026" },
    { id: "P-19", name: "Priya Sen", sex: "Female", age: "31", email: "priya@sen.com", phone: "+91 88776 65544", lastVisit: "May 2, 2026" }
  ]);
        ...
        <div className="space-y-6 px-4">
          <div className="flex items-center gap-3 px-2">
            ...
            <div>
              ...
              <span className="text-[10px] font-bold text-emerald-500 bg-emerald-500/8 px-2 py-0.5 rounded-full uppercase tracking-wider">Specialist</span>
            </div>
          </div>
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto">
        ...
        <header className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Medora Consultation Room portal. View appointments and clinical analytics.</p>
          </div>
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-72 rounded-2xl border bg-card/95 backdrop-blur-md shadow-glow p-3 z-30">
                  <div className="flex justify-between items-center pb-2 border-b mb-2">
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Doctor alerts</p>
                    <button onClick={() => setShowNotifications(false)} className="text-xs text-primary font-semibold hover:underline">Clear all</button>
                  </div>
                  <div className="space-y-2">
                    <div className="p-2 rounded-xl bg-emerald-500/5 border border-emerald-500/20 text-xs">
                      ...
                      <p className="text-muted-foreground mt-0.5">Jane Doe is currently waiting in the video session room.</p>
                      <button
            ...
            <div className="relative">
              ...
              {showProfileDropdown && (
                <div className="absolute right-0 mt-2 w-48 rounded-xl border bg-card shadow-card p-1.5 z-30">
                  ...
                  <button onClick={() => setActiveTab("settings")} className="w-full text-left px-2 py-1.5 rounded-lg text-xs hover:bg-muted transition cursor-pointer">Room Settings</button>
          ...
          {activeTab === "dashboard" && (
            ...
            >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "Today's Consults", val: "3 Appointments", desc: "1 Active, 2 Scheduled", color: "border-primary bg-primary/5", icon: Clock },
                  { label: "Total Patients", val: "148 Cases", desc: "Digital history stored", color: "", icon: Users },
                  { label: "Consultation Revenue", val: "₹18,500", desc: "Monthly payouts active", color: "", icon: DollarSign },
                  { label: "Pending Prescriptions", val: "0 Pending", desc: "All patient files generated", color: "", icon: FileText }
                ].map((stat, i) => {
                  const Icon = stat.icon;
                  return (
                    <div key={i} className={`p-5 bg-card border rounded-3xl shadow-card transition-all relative overflow-hidden group ${stat.color}`}>
              ...
              <div className="grid md:grid-cols-3 gap-4">
                <div className="p-6 rounded-3xl border bg-gradient-to-br from-primary/10 to-primary/5 shadow-card flex flex-col justify-between h-48 relative overflow-hidden group">
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-primary bg-primary/20 px-2.5 py-1 rounded-full uppercase tracking-wider">Launch Room</span>
                    <h3 className="font-bold text-lg mt-3">Start Active Consultation</h3>
                    <p className="text-xs text-muted-foreground mt-1">Open direct HIPAA video room with custom whiteboard, prescription tabs.</p>
                  </div>
                  <button
                    onClick={() => navigate("/doctor/consultation/doctor_45_patient_88_2026")}
                ...
                <div className="p-6 rounded-3xl border bg-card shadow-card flex flex-col justify-between h-48 relative overflow-hidden group">
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-emerald-600 bg-emerald-500/10 px-2.5 py-1 rounded-full uppercase tracking-wider">Patient Care</span>
                    <h3 className="font-bold text-lg mt-3">View Today's Patients</h3>
                    <p className="text-xs text-muted-foreground mt-1">Review medical records, reports, blood panel files, and diagnostic notes.</p>
                  </div>
                  <button
                ...
                <div className="p-6 rounded-3xl border bg-card shadow-card flex flex-col justify-between h-48 relative overflow-hidden group">
                  ...
                  <div>
                    <span className="text-[10px] font-bold text-amber-600 bg-amber-500/10 px-2.5 py-1 rounded-full uppercase tracking-wider">Earnings</span>
                    <h3 className="font-bold text-lg mt-3">Consultation Analytics</h3>
                    <p className="text-xs text-muted-foreground mt-1">Track fee payouts, insurance billing claims, and daily clinic reports.</p>
                  </div>
                  <button
              ...
              <div className="bg-card border rounded-3xl p-6 shadow-card">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Click "Launch Consult Room" to open the video call and Excalidraw session.</p>
                  </div>
                  <span className="text-xs font-bold bg-primary/10 text-primary px-3 py-1 rounded-full">Thursday, May 28</span>
                </div>
                          ...
                          <td className="py-4 pr-4">
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                ...
                                <span className="text-[10px] text-muted-foreground ml-1.5">{apt.age}</span>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="py-4 pl-4 text-right">
                            <button
                              ...
                              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold shadow-soft transition hover:scale-102 cursor-pointer ${
                                apt.status === "Active" ? "bg-gradient-primary text-primary-foreground" : "border hover:bg-muted"
                              }`}
                            >
          ...
          {activeTab === "patients" && (
            ...
            >
              <div className="mb-6">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Digital health database of all patients registered with Medora.</p>
              </div>
                      ...
                      <tr key={pat.id} className="hover:bg-muted/30 transition-colors">
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-2.5">
                            <div className="size-9 rounded-xl bg-primary/10 text-primary grid place-items-center font-bold text-sm shrink-0">{pat.name.charAt(0)}</div>
                            <div>
                              <p className="font-bold">{pat.name}</p>
                              <p className="text-[10px] text-muted-foreground font-mono">{pat.id}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-4 text-muted-foreground">{pat.sex} / {pat.age} yrs</td>
                        <td className="py-4 px-4 text-muted-foreground text-xs">{pat.email}</td>
                        <td className="py-4 px-4 text-muted-foreground text-xs font-mono">{pat.phone}</td>
                        <td className="py-4 px-4 font-semibold text-foreground/80">{pat.lastVisit}</td>
                        <td className="py-4 pl-4 text-right">
                          <button
          ...
          {activeTab === "appointments" && (
            ...
            >
              <div className="mb-6">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Manage details of upcoming clinic bookings.</p>
              </div>

              {/* Repeating simple table but with slot manager */}
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {appointments.map((apt) => (
                  <div key={apt.id} className="p-5 border rounded-2xl bg-card hover:shadow-soft transition-all space-y-4">
                    <div className="flex justify-between items-start">
                      ...
                      <span className={`inline-block text-[9px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                        apt.status === "Active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 animate-pulse" : "bg-blue-500/10 text-blue-600 dark:text-blue-400"
                      }`}>{apt.status}</span>
                    </div>

                    <div className="border-t pt-3 space-y-2">
                      <p className="text-[10px] uppercase font-bold text-muted-foreground">Consultation Reason</p>
                      <p className="text-xs text-foreground/80 leading-relaxed font-medium">{apt.reason}</p>
                    </div>
          ...
          {activeTab === "earnings" && (
            ...
            >
              <div className="bg-card border rounded-3xl p-6 shadow-card">
                ...
                <p className="text-xs text-muted-foreground mt-0.5">Breakdown of telemedicine payout summaries.</p>
                <div className="grid grid-cols-3 gap-4 mt-6">
                  <div className="p-4 border rounded-2xl">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground">Daily Revenue</p>
                    <p className="text-xl font-bold mt-1">₹4,500</p>
                  </div>
                  <div className="p-4 border rounded-2xl">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground">Weekly Revenue</p>
                    <p className="text-xl font-bold mt-1">₹31,500</p>
                  </div>
                  <div className="p-4 border rounded-2xl">
                    <p className="text-[10px] uppercase font-bold text-muted-foreground">Monthly Payout</p>
              ...
              <div className="space-y-4 max-w-xl">
                <div className="flex items-center justify-between p-3 border rounded-xl">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Route through secure clinic network.</p>
                  </div>
                ...
                <div className="flex items-center justify-between p-3 border rounded-xl">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Automated state sync via WebSockets.</p>
                  </div>
                  <span className="text-[10px] font-bold text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-full">ACTIVE</span>
                </div>
              </div>
