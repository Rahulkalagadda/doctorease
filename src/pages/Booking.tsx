...
export default function Booking() {
  ...
  const filtered = doctor
    ? doctors.filter((d) => d.name === doctor)
    : doctors.filter((d) => !spec || d.spec === spec);
    ...
    <>
      <section className="pt-28 md:pt-32 pb-6 bg-hero">
        <div className="container-px mx-auto max-w-5xl text-center">
          <span className="inline-flex text-xs font-medium uppercase tracking-wider text-primary bg-primary/10 px-3 py-1 rounded-full">
            Book in 60 seconds
          </span>
          ...
          <p className="mt-3 text-muted-foreground text-sm sm:text-base">
            Real-time availability — same-day slots included.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="glass rounded-2xl p-3 md:p-4 shadow-card mb-8">
            <div className="text-center mb-3 md:hidden">
              ...
              <p className="text-sm font-bold text-foreground mt-0.5">{steps[step]}</p>
            </div>
              ...
              {steps.map((s, i) => {
                ...
                return (
                  <div
                    key={s}
                    className={cn(
                      "flex items-center gap-1 md:gap-2 min-w-0",
                  ...
                  >
                    ...
                    <span
                      className={cn(
                        "text-xs md:text-sm font-medium truncate hidden md:block",
              ...
              {done ? (
                ...
                >
                  ...
                  <p className="mt-2 text-muted-foreground">
                    ...
                    <span className="font-medium text-foreground">{form.email}</span> and via SMS to{" "}
                    {form.phone}.
                  </p>
                  <div className="mt-6 inline-block text-left bg-muted/60 rounded-2xl p-5">
                    <p className="text-sm">
                      <span className="text-muted-foreground">Specialty:</span> <b>{spec}</b>
                    </p>
                    <p className="text-sm mt-1">
                      <span className="text-muted-foreground">Doctor:</span> <b>{doctor}</b>
                    </p>
                    <p className="text-sm mt-1">
                ...
                >
                  {step === 0 && (
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
                        <Sparkles className="size-5 text-primary" /> What do you need help with?
                      </h2>
                      <p className="text-muted-foreground text-sm sm:text-base mt-1">
                        Choose a specialty to see matching doctors.
                      </p>
                            ...
                            <button
                              ...
                              className={cn(
                                ...
                                spec === s
                                  ...
                                  : "hover:bg-muted hover:shadow-soft hover:-translate-y-1",
                              )}
                            >
                              <div className="flex items-start justify-between w-full">
                                <div
                                  className={cn(
                                    "size-9 sm:size-10 rounded-xl grid place-items-center transition-transform group-hover:scale-110",
                                    spec === s
                              ...
                              <div>
                                <p className="font-bold text-foreground text-xs sm:text-sm tracking-tight">
                                  {s}
                                </p>
                  ...
                  {step === 1 && (
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
                        <User className="size-5 text-primary" /> Pick a doctor
                      </h2>
                      <p className="text-muted-foreground text-sm sm:text-base mt-1">
                      ...
                      <div className="mt-6 grid sm:grid-cols-2 gap-3">
                        {filtered.map((d) => (
                          ...
                          >
                            <div className="size-12 rounded-xl bg-gradient-primary text-primary-foreground grid place-items-center font-bold shrink-0">
                              {d.name
                                .split(" ")
                                .map((n) => n[0])
                                .slice(0, 2)
                  ...
                  {step === 2 && (
                    <div>
                      <h2 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
                        <Calendar className="size-5 text-primary" /> Choose date & time
                      </h2>
                      <p className="text-muted-foreground text-sm sm:text-base mt-1">
                                ...
                                className={cn(
                                  ...
                                  off
                                    ...
                                    : active
                                      ...
                                      : "hover:bg-muted hover:border-primary/50 hover:shadow-sm",
                                )}
                              >
                                {s}
                              </button>
                  ...
                  {step === 3 && (
                    <div>
                      ...
                      <p className="text-muted-foreground text-sm sm:text-base mt-1">
                        We'll only use this for your appointment.
                      </p>
                      ...
                      <div className="mt-6 grid grid-cols-2 gap-3">
                        {[
                          ...
                          <div
                            key={r.k}
                            className={cn(
                              "rounded-2xl border p-3 md:p-4 bg-muted/30",
                              r.full ? "col-span-2" : "col-span-1",
                            )}
                          >
