...
import {
  ...
  Upload, Settings, Bell, Search, LogOut, DollarSign, Users, Award, ShieldAlert, Check, X, ArrowUpRight
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "doctors" | "users" | "payments">("dashboard");
  ...
  const [doctorsList, setDoctorsList] = useState([
    { id: "D-201", name: "Dr. Maya Patel", spec: "General Medicine", status: "Active", experience: "12 yrs", rating: "4.9", consultations: "84" },
    { id: "D-202", name: "Dr. Rajesh Sharma", spec: "Cardiology", status: "Active", experience: "18 yrs", rating: "5.0", consultations: "112" },
    { id: "D-203", name: "Dr. Priya Nair", spec: "Neurology", status: "Pending Approval", experience: "11 yrs", rating: "N/A", consultations: "0" }
  ]);

  // Simulated Users State
  const [usersList, setUsersList] = useState([
    { id: "U-1001", name: "Jane Doe", role: "Patient", email: "jane@example.com", joined: "May 2026", status: "Active" },
    { id: "U-1002", name: "Rahul Gupta", role: "Patient", email: "rahul@gupta.com", joined: "April 2026", status: "Active" },
    { id: "U-1003", name: "Spam User", role: "Patient", email: "spam@bot.com", joined: "May 2026", status: "Suspended" }
  ]);
  ...
  const handleUserStatusToggle = (id: string, currentStatus: string) => {
    ...
    setUsersList(usersList.map(user => user.id === id ? { ...user, status: nextStatus } : user));
      ...
      <aside className="w-full md:w-64 bg-card border-r border-border shrink-0 flex flex-col justify-between py-6 md:sticky md:top-0 md:h-screen z-20">
        <div className="space-y-6 px-4">
          <div className="flex items-center gap-3 px-2">
            ...
            <div>
              <p className="font-semibold text-sm text-foreground leading-tight truncate max-w-[140px]">Medora Admin</p>
              <span className="text-[10px] font-bold text-amber-500 bg-amber-500/8 px-2 py-0.5 rounded-full uppercase tracking-wider">Super Administrator</span>
            </div>
          </div>
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto">
        ...
        <header className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Control center for doctor verification, consultation analytics, and system configurations.</p>
          </div>
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-72 rounded-2xl border bg-card shadow-glow p-3 z-30">
                  <div className="flex justify-between items-center pb-2 border-b mb-2">
                    <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Admin Notifications</p>
                  </div>
                  <div className="space-y-2">
                    <div className="p-2 rounded-xl bg-amber-500/5 border border-amber-500/20 text-xs">
                      ...
                      <button onClick={() => { setActiveTab("doctors"); setShowNotifications(false); }} className="mt-1 font-bold text-primary hover:underline">Verify Documents</button>
                    </div>
                  </div>
                </div>
              )}
        ...
        <AnimatePresence mode="wait">
          {activeTab === "dashboard" && (
            ...
            >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "Active Doctors", val: `${doctorsList.filter(d=>d.status==="Active").length} Certified`, desc: "1 pending review", color: "border-primary bg-primary/5", icon: Stethoscope },
                  { label: "Total Registered Patients", val: "1,240 Users", desc: "Digital profiles encrypted", color: "", icon: Users },
                  { label: "Platform Revenue", val: "₹4,28,000", desc: "Commission: 15% active", color: "", icon: DollarSign },
                  { label: "Consultation Calls", val: "354 Rooms", desc: "99.98% uptime Jitsi", color: "", icon: Activity }
                ].map((stat, i) => {
              ...
              <div className="bg-card border rounded-3xl p-6 shadow-card">
                <h3 className="font-semibold text-base mb-4">Urgent Approvals: Doctor Registrations</h3>
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm border-collapse">
                    ...
                    <tbody className="divide-y">
                      {doctorsList.filter(doc => doc.status === "Pending Approval").map((doc) => (
                        <tr key={doc.id} className="hover:bg-muted/30 transition-colors">
                          <td className="py-4 pr-4">
                            <div className="flex items-center gap-2">
                              <div className="size-8 rounded-lg bg-amber-500/10 text-amber-600 grid place-items-center font-bold text-xs">{doc.name.charAt(4)}</div>
                              <span className="font-bold text-foreground">{doc.name}</span>
                            </div>
                          </td>
                          <td className="py-4 px-4 text-muted-foreground">{doc.spec}</td>
                          <td className="py-4 px-4 font-semibold text-foreground/85">{doc.experience}</td>
                      ...
                      {doctorsList.filter(doc => doc.status === "Pending Approval").length === 0 && (
                        <tr>
                      ...
                      <tr key={doc.id} className="hover:bg-muted/30 transition-colors">
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-2.5">
                            <div className="size-9 rounded-xl bg-primary/10 text-primary grid place-items-center font-bold text-sm shrink-0">{doc.name.charAt(4)}</div>
                            <div>
                              <p className="font-bold">{doc.name}</p>
                              <p className="text-[10px] text-muted-foreground font-mono">{doc.id} · Consultations: {doc.consultations}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-4 text-muted-foreground font-medium">{doc.spec}</td>
                        <td className="py-4 px-4 font-semibold text-foreground/80">{doc.experience}</td>
                        ...
                        <td className="py-4 px-4 text-center">
                          <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                            doc.status === "Active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-amber-500/10 text-amber-600"
                          }`}>{doc.status}</span>
                        </td>
                        <td className="py-4 pl-4 text-right">
                          {doc.status !== "Active" ? (
                            ...
                            <button
                              onClick={() => {
                                setDoctorsList(doctorsList.map(d => d.id === doc.id ? { ...d, status: "Deactivated" } : d));
                  ...
                  <tbody className="divide-y">
                    {usersList.map((user) => (
                      <tr key={user.id} className="hover:bg-muted/30 transition-colors">
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-2.5">
                            <div className="size-9 rounded-xl bg-slate-100 dark:bg-slate-800 text-foreground grid place-items-center font-bold text-sm shrink-0">{user.name.charAt(0)}</div>
                            <div>
                        ...
                        <td className="py-4 px-4 font-semibold text-foreground/80">{user.joined}</td>
                        <td className="py-4 px-4 text-center">
                          <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                            user.status === "Active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-red-500/10 text-red-500"
                          }`}>{user.status}</span>
                        </td>
                        <td className="py-4 pl-4 text-right">
                          <button
                            ...
                            className={`px-2.5 h-8 border rounded-lg text-xs font-semibold cursor-pointer ${
                              user.status === "Active" ? "hover:bg-red-500/10 hover:text-red-500" : "hover:bg-emerald-500/10 hover:text-emerald-500"
                            }`}
                          >
          ...
          {activeTab === "payments" && (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground mb-6">Digital record of consultations, insurance commissions and doctor balance payouts.</p>
              <div className="space-y-4">
                {[
                  { label: "Commission Payout Vashi Clinic", val: "₹12,450", date: "Today", status: "Settled" },
                  { label: "Doctor Maya Patel - Consultation Fees Payout", val: "₹18,500", date: "Yesterday", status: "Settled" },
                  { label: "Doctor Rajesh Sharma - Payout Transferred", val: "₹24,000", date: "May 25", status: "Settled" }
                ].map((pay, i) => (
                  <div key={i} className="flex justify-between items-center p-3 border rounded-xl bg-slate-50 dark:bg-card">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-[10px] text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-full font-bold uppercase">{pay.status}</span>
                    </div>
                  </div>
                ))}
              </div>
