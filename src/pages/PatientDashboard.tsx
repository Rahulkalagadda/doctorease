...
import {
  ...
  Baby, Bone, Eye, Sparkles, Phone, MessageSquare, Activity, LayoutDashboard, FileText,
  Upload, Settings, Bell, Search, LogOut, Download, FileUp, X, Check, Clock, User
} from "lucide-react";
...
export default function PatientDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"dashboard" | "appointments" | "prescriptions" | "reports" | "settings">("dashboard");
  ...
  const [appointments, setAppointments] = useState([
    { id: "1", doctor: "Dr. Maya Patel", spec: "General Medicine", date: "Today, May 28", time: "03:40 PM", status: "Active", roomId: "doctor_45_patient_88_2026" },
    { id: "2", doctor: "Dr. Rajesh Sharma", spec: "Cardiology", date: "June 2, 2026", time: "10:00 AM", status: "Scheduled", roomId: "doctor_12_patient_88_2026" },
    { id: "3", doctor: "Dr. Aisha Khan", spec: "Pediatrics", date: "May 15, 2026", time: "11:30 AM", status: "Completed", roomId: "doctor_8_patient_88_2026" }
  ]);

  // Simulated Prescriptions State
  const [prescriptions, setPrescriptions] = useState([
    { id: "P-101", date: "May 15, 2026", doctor: "Dr. Aisha Khan", diagnosis: "Allergic Rhinitis", medicines: ["Cetirizine 10mg (Once daily)", "Fluticasone Nasal Spray (Twice daily)"] },
    { id: "P-98", date: "April 2, 2026", doctor: "Dr. Maya Patel", diagnosis: "Acute Pharyngitis", medicines: ["Amoxicillin 500mg (Thrice daily)", "Paracetamol 650mg (As needed)"] }
  ]);

  // Simulated Reports State
  const [reports, setReports] = useState([
    { id: "R-501", name: "Blood_Panel_Report.pdf", type: "PDF", size: "1.2 MB", date: "May 14, 2026" },
    { id: "R-402", name: "Chest_XRay_Digital.jpg", type: "Image", size: "4.8 MB", date: "April 18, 2026" }
  ]);
      ...
      <aside className="w-full md:w-64 bg-card border-r border-border shrink-0 flex flex-col justify-between py-6 md:sticky md:top-0 md:h-screen z-20">
        <div className="space-y-6 px-4">
          <div className="flex items-center gap-3 px-2">
            ...
            <div>
              <p className="font-semibold text-sm text-foreground leading-tight truncate max-w-[140px]">{userName}</p>
              <span className="text-[10px] font-bold text-primary bg-primary/8 px-2 py-0.5 rounded-full uppercase tracking-wider">Patient</span>
            </div>
          </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition cursor-pointer ${
                    active ? "bg-gradient-primary text-primary-foreground shadow-soft" : "text-foreground/75 hover:bg-muted hover:text-foreground"
                  }`}
                >
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto">
        ...
        <header className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Medora HIPAA-secured patient consultation room dashboard.</p>
          </div>
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-72 rounded-2xl border bg-card/95 backdrop-blur-md shadow-glow p-3 z-30">
                  <div className="flex justify-between items-center pb-2 border-b mb-2">
                    ...
                    <button onClick={() => setShowNotifications(false)} className="text-xs text-primary font-semibold hover:underline">Mark read</button>
                  </div>
                  <div className="space-y-2">
                    <div className="p-2 rounded-xl bg-primary/5 border text-xs">
                      ...
                      <p className="text-muted-foreground mt-0.5">Your video session with Dr. Maya Patel starts in 10 minutes!</p>
                      <button
              ...
              {showProfileDropdown && (
                <div className="absolute right-0 mt-2 w-48 rounded-xl border bg-card shadow-card p-1.5 z-30">
                  <div className="px-2 py-1.5 border-b mb-1">
                    ...
                    <p className="text-[10px] text-muted-foreground truncate">patient@example.com</p>
                  </div>
                  <button onClick={() => setActiveTab("settings")} className="w-full text-left px-2 py-1.5 rounded-lg text-xs hover:bg-muted transition cursor-pointer">Profile Settings</button>
                  <button onClick={handleLogout} className="w-full text-left px-2 py-1.5 rounded-lg text-xs hover:bg-muted text-destructive font-semibold transition cursor-pointer">Sign Out</button>
                </div>
              )}
            </div>
          </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === "dashboard" && (
            ...
            >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "Active Consultations", val: "1 Call Today", desc: "Dr. Maya Patel · 03:40 PM", color: "border-primary bg-primary/5" },
                  { label: "Total Appointments", val: "3 Booked", desc: "1 today, 1 scheduled, 1 past", color: "" },
                  { label: "Latest Prescription", val: "Cetirizine 10mg", desc: "Prescribed 13 days ago", color: "" },
                  { label: "Uploaded Reports", val: `${reports.length} Documents`, desc: "X-ray, blood panel digital files", color: "" }
                ].map((stat, i) => (
                  <div key={i} className={`p-4 bg-card border rounded-2xl shadow-card transition-all ${stat.color}`}>
              ...
              <div className="bg-gradient-primary rounded-3xl p-5 md:p-6 text-primary-foreground shadow-glow flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                  <span className="inline-flex text-[10px] font-bold uppercase tracking-wider text-emerald-300 bg-emerald-500/20 px-2 py-0.5 rounded-full animate-pulse">Live Meeting Available</span>
                  <h2 className="text-xl md:text-2xl font-bold mt-2">Consultation Room Is Open!</h2>
                  <p className="text-sm opacity-90 mt-1">Your video visit with <b>Dr. Maya Patel</b> is ready. Collaborative whiteboard & prescription tools initialized.</p>
                </div>
                <button
                  onClick={() => navigate("/doctor/consultation/doctor_45_patient_88_2026")}
              ...
              <div className="bg-card border rounded-3xl p-6 shadow-card">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-0.5">Real-time schedule of your online clinic appointments.</p>
                  </div>
                  <Link to="/booking">
                    <Button size="sm" className="rounded-full bg-gradient-primary text-primary-foreground">Book New</Button>
                  </Link>
                </div>
                        ...
                        <tr key={apt.id} className="group hover:bg-muted/30 transition-colors">
                          <td className="py-4 pr-4">
                            <div className="flex items-center gap-2">
                              <div className="size-8 rounded-lg bg-primary/10 text-primary grid place-items-center font-bold text-xs">{apt.doctor.split(" ")[1]?.[0]}</div>
                              <span className="font-semibold">{apt.doctor}</span>
                            </div>
                          </td>
                          <td className="py-4 px-4 text-muted-foreground">{apt.spec}</td>
                          <td className="py-4 px-4 font-medium text-foreground/80">{apt.date} · {apt.time}</td>
                          <td className="py-4 px-4 text-center">
                            <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                              apt.status === "Active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 animate-pulse" :
                          ...
                          <td className="py-4 pl-4 text-right">
                            {apt.status === "Active" ? (
                              ...
                              <span className="text-xs text-muted-foreground font-medium">Link ready 10m before</span>
                            ) : (
                              <button
            ...
            >
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Browse through upcoming, active and historical video consults.</p>
                </div>
                <Link to="/booking">
                  <Button size="sm" className="rounded-full bg-gradient-primary">Book Appointment</Button>
                </Link>
              </div>
                      ...
                      <tr key={apt.id} className="group hover:bg-muted/30 transition-colors">
                        <td className="py-4 pr-4">
                          <div className="flex items-center gap-2.5">
                            <div className="size-9 rounded-xl bg-primary/10 text-primary grid place-items-center font-bold text-sm shrink-0">{apt.doctor.split(" ")[1]?.[0]}</div>
                            <div>
                        ...
                        <td className="py-4 px-4 font-semibold text-foreground/80">{apt.date} · {apt.time}</td>
                        <td className="py-4 px-4 font-mono text-xs text-muted-foreground select-all">{apt.roomId}</td>
                        <td className="py-4 px-4 text-center">
                          <span className={`inline-block text-[10px] px-2 py-0.5 rounded-full font-bold uppercase tracking-wider ${
                            apt.status === "Active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 animate-pulse" :
                            apt.status === "Scheduled" ? "bg-blue-500/10 text-blue-600 dark:text-blue-400" :
                        ...
                        <td className="py-4 pl-4 text-right">
                          {apt.status === "Active" ? (
                            ...
                            <div className="flex justify-end gap-2">
                              <button onClick={() => toast.success("Simulating call reschedule")} className="text-xs text-primary hover:underline font-semibold">Reschedule</button>
                              <span className="text-muted-foreground">|</span>
                              <button onClick={() => { setAppointments(appointments.filter(a => a.id !== apt.id)); toast.error("Appointment cancelled."); }} className="text-xs text-destructive hover:underline font-semibold">Cancel</button>
                            </div>
                          ) : (
                            <div className="flex justify-end gap-2">
                              <button onClick={() => setActiveTab("prescriptions")} className="text-xs text-primary font-bold hover:underline">Prescription</button>
                              <span className="text-muted-foreground">|</span>
                              <button onClick={() => navigate(`/doctor/consultation/${apt.roomId}`)} className="text-xs text-muted-foreground font-semibold hover:text-foreground">Revisit Room</button>
                            </div>
                          )}
                        </td>
                      </tr>
          ...
          {activeTab === "prescriptions" && (
            ...
            >
              <div className="flex justify-between items-center mb-2">
                <div>
                  <h3 className="font-semibold text-lg text-foreground">Digital Prescription Records</h3>
                  <p className="text-xs text-muted-foreground mt-0.5">HIPAA compliant electronic prescriptions generated by your doctor during consultations.</p>
                </div>
              </div>

              <div className="grid md:grid-cols-2 gap-4">
                {prescriptions.map((pr) => (
                  <div key={pr.id} className="bg-card border rounded-3xl p-6 shadow-card hover:shadow-glow transition-all relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full uppercase tracking-wider">{pr.id}</span>
                    ...
                    <div className="mt-4 border-t pt-4">
                      <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Diagnosis</p>
                      <p className="text-sm font-semibold text-foreground/80 mt-1">{pr.diagnosis}</p>
                    </div>

                    <div className="mt-4">
                      <p className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Prescribed Medicines</p>
                      <ul className="mt-2 space-y-1.5">
                        {pr.medicines.map((med, idx) => (
                          <li key={idx} className="flex gap-2 text-xs text-foreground/80"><Check className="size-4 text-emerald-500 shrink-0" /> {med}</li>
                        ))}
                      </ul>
                    </div>
          ...
          {activeTab === "reports" && (
            ...
            >
              ...
              <div className="bg-card border rounded-3xl p-6 md:p-8 shadow-card text-center relative overflow-hidden">
                ...
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-sm text-muted-foreground mt-1.5 leading-relaxed">
                    Share blood panels, X-rays, MRI scans or past records with your doctor. File uploads are instantly shared in the active Consultation Room.
                  </p>
                  ...
                  <p className="text-[10px] text-muted-foreground mt-3">Accepts PDF, PNG, JPG files up to 25MB. Fully encrypted.</p>
                </div>

                {isUploading && (
                  <div className="mt-6 max-w-xs mx-auto space-y-1.5">
                    <div className="flex justify-between items-center text-xs">
                      <span className="font-semibold text-primary">Uploading medical report...</span>
                      <span>{uploadProgress}%</span>
                    </div>
                    <div className="w-full h-1.5 bg-muted rounded-full overflow-hidden">
                      <div className="h-full bg-gradient-primary transition-all duration-200" style={{ width: `${uploadProgress}%` }} />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="bg-card border rounded-3xl p-6 shadow-card">
                <div className="mb-4">
                  <h3 className="font-semibold text-base">Your Uploaded Documents ({reports.length})</h3>
                  <p className="text-xs text-muted-foreground mt-0.5">Documents you uploaded that are accessible to doctors during consultations.</p>
                </div>
                  ...
                  {reports.map((rep) => (
                    <div key={rep.id} className="py-3 flex items-center justify-between gap-4 group">
                      <div className="flex items-center gap-3">
                        <div className="size-10 rounded-xl bg-teal/10 text-teal-600 grid place-items-center font-bold text-xs shrink-0">{rep.type}</div>
                        <div>
                          <p className="text-sm font-semibold text-foreground/90 truncate max-w-[200px] sm:max-w-md">{rep.name}</p>
                          <p className="text-[10px] text-muted-foreground">{rep.size} · Uploaded {rep.date}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
